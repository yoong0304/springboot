...
public class BoardApiController {
    private final BoardService boardService;    // Autowired 로 빈에 등록
    ...
    @GetMapping(value = "/api/board-detail/{boardId}")
    ...
    public ResponseEntity create_board(@RequestBody BoardDto boardDto) {
        ...
        HttpStatus status = HttpStatus.CREATED; // 201 잘 생성 되었음 을 의미
        try {
    ...
    public ResponseEntity update_board(@RequestBody BoardDto boardDto) {
        ...
        HttpHeaders headers = new HttpHeaders();
        Map<String, String> body = new HashMap<>();
        HttpStatus status = HttpStatus.NO_CONTENT; // 204 수정이 정상적 으로 완료 되었음 을 의미
        try {
